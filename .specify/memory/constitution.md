...
Each agent operates only within its defined scope. The spec-guardian-agent blocks implementation without specs, backend-agent handles FastAPI and SQLModel, frontend-agent handles Next.js UI, auth-integration-agent handles JWT bridge logic, database-agent manages schema and data integrity, and qa-validation-agent validates correctness and security.
...
All development must use the approved technology stack: Frontend: Next.js 16+ (App Router), TypeScript, Tailwind CSS; Backend: Python FastAPI; ORM: SQLModel; Database: Neon Serverless PostgreSQL; Authentication: Better Auth (frontend) + JWT verification (backend).
