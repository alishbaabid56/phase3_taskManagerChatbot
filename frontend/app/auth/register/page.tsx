...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Registration will automatically log the user in via the AuthProvider
      ...
      if (err instanceof Error) {
        ...
        if (err.hasOwnProperty('message')) {
          ...
          setError((err as any).detail as string || 'Registration failed');
        } else if (err.hasOwnProperty('error')) {
          setError((err as any).error as string || 'Registration failed');
        } else {
      ...
      <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8">
          ...
          <div className="text-center text-sm text-gray-400">
            ...
            <Link href="/auth/login" className="font-medium text-sky-400 hover:text-sky-300 hover:underline transition-all duration-300 ease-in-out">
              Sign in
            </Link>
          </div>
        </div>
